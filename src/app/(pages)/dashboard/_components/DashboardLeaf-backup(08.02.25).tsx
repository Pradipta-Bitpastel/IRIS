...
import { SelectedValues } from "../../alertsetting/types/type";
...
const DashboardLeaf = memo(() => {
  ...
  const [isAlertMessageDataPresent, setIsAlertMessageDataPresent] = useState<number>(0);
  ...
  const latestThreeRef = useRef([]);
  const fetchData = async (newOffset) => {
    console.log(`Fetching data with offset: ${newOffset}`);
    try {
      const storedData = window.localStorage.getItem("sessionCheck");
      const decryptUserID = decryptPassword(JSON.parse(storedData).userID);
      const alertPayload = generatingPayloadForAlertApi(newOffset, alertMsgLimit, decryptUserID);
      // const response = await fetch(`${apiUrl}?limit=${limit}&offset=${newOffset}`);
      const result = await alertApi(alertPayload);
      

      if (result?.data?.result.length > 0) {
        setData((prev) => [...prev, ...result?.data?.result]);
        // setOffset(offset + alertMsgLimit);
        console.log(`Fetched ${result?.data?.result.length} new items.`);
      }
  ...
  // Load more data when reaching the 6th item
  ...
  //Souvik
  const generatingPayloadForApi = (offset: number = 0, limit: number = chatMsgLimit, user_id: String = undefined) => {
    const formData = new FormData();
    ...
    if (user_id) {
      ...
      // formData.append("limit", String(Number(limit)-15));
    }
    // else{
    //   formData.append("limit", String(limit));
    // }
  ...
  const generatingPayloadForAlertApi = (offset: number = 0, limit: number = alertMsgLimit, user_id: String = undefined) => {
    const formData = new FormData();
  ...
  const alertApi = (payload) => {
    try {
      ...
      toast.error(allowableServerErrors.includes(errorMessage) ? errorMessage : generalizedApiError, toastErrorSettingDataCpy);
    }
  };
  //souvik
  ...
  useEffect(() => {
    const userID = getlLocalStorageData();
    ...
    const postLogMsgApiRequest = async () => {
      ...
      const alertPayload = generatingPayloadForApi(chatMsgOffsetCount, alertMsgLimit, userID);
      try {
        ...
        // const alertMsgData: any = await alertApi(alertPayload);
        ...
        // setIsAlertMessageDataPresent(alertMsgData?.data?.alert_status ? Number(alertMsgData?.data?.alert_status) : 0);
  ...
  useEffect(() => {
    ...
    intervalRef.current = setInterval(() => {
      ...
      latestThreeRef.current = [...latestThreeRef.current, data[nextIndex]];
      if (latestThreeRef.current.length > 3) latestThreeRef.current.shift();

      console.log("Updated latestThree:", latestThreeRef.current);

      // Fetch more data when reaching the 6th item
      if (latestThreeRef.current.length === 6) {
        console.log("6th item pushed! Fetching more data...");
  ...
  }, [data, currentIndex]); // Dependencies are only `data` and `currentIndex`
              ...
              <div className="col-lg-3 col-md-6 order-3 order-lg-3 mt-3 mt-lg-0">
                ...
                <div className="dashboard_main_left_section right_div">
                  <div className="left_card">
                    ...
                    <div className="left_card_body alert-section" key={12}>
                      {
                        latestThree?.map((msg_item, msg_item_index) =>
                          msg_item_index <= 2 ? (
                            <>
                              <Link
                                href={{
                              ...
                              >
                                <div className="inner_card">
                                  <div className="inner_card_title">
                                    ...
                                    <span className="inner_card_date">{dateFormatter(msg_item?.chat_time)}</span>
                                  </div>
                                  <div className="inner_card_body">
                                    <div className="inner_card_body_maincontent">
                                      <p>{msg_item?.message_text.length > 70 ? `${msg_item?.message_text.slice(0, 70)} ...` : msg_item?.message_text}</p>
                                    ...
                                    {msg_item?.alpha2_code && (
                                      <div className="inner_card_country">
                                        <div className="inner_card_country_svg">
                                          <img src={`http://purecatamphetamine.github.io/country-flag-icons/3x2/${msg_item?.alpha2_code || "US"}.svg`} className="img-fluid"></img>
                                        </div>
                                        {msg_item && msg_item?.entity?.country && <span>{msg_item?.entity?.country?.name}</span>}
                                      </div>
                                    )}
                                  </div>
                                </div>
                      ...
                      {/* {isAlertMessageDataPresent === 1
                        ? alertMsg &&
                      
                        : <div className="no-data-section">{isAlertMessageDataPresent === 0 ? `No data available` : (isAlertMessageDataPresent === 5 ? `No custom alert available` : ``)}</div>} */}
                    </div>
