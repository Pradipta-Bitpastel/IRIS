...
export const translateText = async (text: string, messageId: string): Promise<string | void> => {
  ...
  // Update the status to indicate translation is in progress
  // setTranslatedTexts(prevState => ({
  //   ...prevState,
  //   [messageId]: { message: '', status: true }
  // }));
  ...
  try {
    ...
    // After translation is complete, update the translated text and status
    // setTranslatedTexts(prevState => ({
    //   // ...prevState,
    //   [messageId]: { message: translatedText, status: false }
    // }));
  } catch (error) {
